...
class BinaryPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        logits, features = net(data, return_feature=True)
        # TODO: get top k prediction
        pred = torch.argmax(logits.detach(), dim=1)
        feat1, feat2 = get_reference_eval_pred(features, pred, self.all_feats, self.all_labels, "cosine_similarity")
        conf = net([feat1, feat2], return_score=True)
        return pred, conf
