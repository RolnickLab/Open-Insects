...
class OpenMax(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data):
        ...
        for score in scores:
            so, _ = openmax(
                self.weibull_model, categories, score, 0.5, self.weibull_alpha, "euclidean"
            )  # openmax_prob, softmax_prob
...
def compute_channel_distances(mavs, features, eu_weight=0.5):
    ...
    for channel, mcv in enumerate(mavs):  # Compute channel specific distances
        ...
        eucos_dists.append(
            [spd.euclidean(mcv, feat[channel]) * eu_weight + spd.cosine(mcv, feat[channel]) for feat in features]
        )
...
def compute_train_score_and_mavs_and_dists(train_class_num, trainloader, device, net):
    ...
    with torch.no_grad():
        for train_step in tqdm(range(1, len(train_dataiter) + 1), desc="Progress: ", position=0, leave=True):
    ...
    #     try:
    #         scores[i] = torch.cat(x).cpu().numpy()
    #     except RuntimeError:
    #         scores[i] = np.zeros(
    #             (3, 1, train_class_num)
