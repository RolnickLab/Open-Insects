...
class GRAMPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        preds, deviations = get_deviations(net, data, self.feature_min, self.feature_max, self.num_classes, self.powers)
...
def sample_estimator(model, train_loader, num_classes, powers):
    ...
    feature_class = [[[None for x in range(num_poles)] for y in range(num_layer)] for z in range(num_classes)]
    label_list = []
    mins = [[[None for x in range(num_poles)] for y in range(num_layer)] for z in range(num_classes)]
    maxs = [[[None for x in range(num_poles)] for y in range(num_layer)] for z in range(num_classes)]
    ...
    for label in range(num_classes):
        for layer_idx in range(num_layer):
            for poles_idx in range(num_poles):
                feature = torch.tensor(np.array(feature_class[label][layer_idx][poles_idx]))
                ...
                if mins[label][layer_idx][poles_idx] is None:
                    ...
                    mins[label][layer_idx][poles_idx] = torch.min(current_min, mins[label][layer_idx][poles_idx])
                    maxs[label][layer_idx][poles_idx] = torch.max(current_min, maxs[label][layer_idx][poles_idx])
