...
class KLMatchingPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            preds = []
            with torch.no_grad():
                for batch in tqdm(id_loader_dict["val"], desc="Setup: ", position=0, leave=True):
            ...
            for i in tqdm(range(self.num_classes)):
                ...
                if torch.sum(preds.eq(i).float()) == 0:
                    ...
                    self.mean_softmax_val.append(all_softmax[preds.eq(i)].mean(0).numpy())
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        scores = -pairwise_distances_argmin_min(softmax, np.array(self.mean_softmax_val), metric=self.kl)[1]
