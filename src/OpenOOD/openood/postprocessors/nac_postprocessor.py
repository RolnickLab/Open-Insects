# from https://github.com/BierOne/ood_coverage/blob/master/openood/postprocessors/nac_postprocessor.py
...
class NACPostprocessor(BasePostprocessor):
    ...
    def load(self, f_name, unpack, params=None):
        self.Coverage = KMNC.load(self.save_dir, f_name, unpack=unpack, params=params, verbose=False)

    def save(self, prefix="imagenet"):
        os.makedirs(self.save_dir, exist_ok=True)
        self.Coverage.save(self.save_dir, prefix=prefix, aka_ln=self.ln_to_aka, verbose=False)
    ...
    def build_nac(self, net: nn.Module, reload=False, prefix="imagenet"):
        ...
        if reload and os.path.isfile(os.path.join(self.save_dir, f_name)):
            ...
            self.Coverage = KMNC(self.layer_size_dict, hyper=self.layer_kwargs, unpack=unpack)
            if self.use_cache:
                self.Coverage.assess_with_cache(self.nac_dataloader)
            else:
                self.Coverage.assess(net, self.nac_dataloader, self.spatial_func)
    ...
    def setup(
        ...
        **kwargs,
        ...
        if self.use_cache:
            ...
            self.nac_dataset = TrainSubset(
                id_loader_dict["main_train"].dataset, valid_num=self.valid_num, balanced=True
            )
            self.nac_dataloader = DataLoader(
                self.nac_dataset, batch_size=64, shuffle=False, num_workers=8, pin_memory=True, drop_last=False
            )

            dummy_shape = self.nac_dataset.dataset[0]["data"].shape
        print("Input shape:", dummy_shape)
        self.layer_size_dict = make_layer_size_dict(
            net, self.layer_names, input_shape=(3, *dummy_shape), spatial_func=self.spatial_func
        )
    ...
    def inference(self, net, data_loader, progress=True):
        if self.use_cache:
            confs, flags, preds, labels = self.Coverage.assess_ood_with_cache(data_loader, progress=progress)
        else:
            confs, flags, preds, labels = self.Coverage.assess_ood(
                net, data_loader, self.spatial_func, progress=progress
            )
    ...
    def get_hyperparam(self):
        print_str = ""
        for ln in self.layer_kwargs:
            print_str += "\n{} paramters O:{}, M:{}, " "sig_alpha:{}, method:{}".format(
                ln, *list(self.layer_kwargs[ln].values())
            )
        return print_str
