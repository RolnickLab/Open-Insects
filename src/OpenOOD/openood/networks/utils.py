...
from .osr_branch_net import OSRBranchNet
...
def get_network(network_config):
    ...
    if network_config.name == "resnet18_32x32":
        ...
        net = WideResNet(depth=28, widen_factor=10, dropRate=0.0, num_classes=num_classes)

    elif network_config.name == "densenet":
        net = DenseNet3(
            depth=100, growth_rate=12, reduction=0.5, bottleneck=True, dropRate=0.0, num_classes=num_classes
        )
        ...
        net = CIDERNet(
            backbone=backbone, head=network_config.head, feat_dim=network_config.feat_dim, num_classes=num_classes
        )
        ...
        net = RTSNet(backbone=backbone, feature_size=feature_size, num_classes=num_classes, dof=network_config.dof)
        ...
        net = get_csi_linear_layers(
            feature_size, num_classes, network_config.simclr_dim, network_config.shift_trans_type
        )
        ...
        model = ReconstructiveSubNetwork(in_channels=3, out_channels=3, base_width=int(network_config.image_size / 2))
        model_seg = DiscriminativeSubNetwork(
            in_channels=6, out_channels=2, base_channels=int(network_config.image_size / 4)
        )
        ...
        net = UDGNet(backbone=backbone, num_classes=num_classes, num_clusters=network_config.num_clusters)

    elif network_config.name == "opengan":
        from .opengan import Discriminator, Generator

        backbone = get_network(network_config.backbone)
        netG = Generator(in_channels=network_config.nz, feature_size=network_config.ngf, out_channels=network_config.nc)
        netD = Discriminator(in_channels=network_config.nc, feature_size=network_config.ndf)

        net = {"netG": netG, "netD": netD, "backbone": backbone}

    elif network_config.name == "arpl_gan":
        from .arpl_net import resnet34ABN, Generator, Discriminator, Generator32, Discriminator32, ARPLayer
        ...
        criterion = ARPLayer(
            feat_dim=dim_centers, num_classes=num_classes, weight_pl=network_config.weight_pl, temp=network_config.temp
        )

        assert (
            network_config.image_size == 32 or network_config.image_size == 64
        ...
        net = KNOWN_MODELS[network_config.model](
            head_size=network_config.num_logits, zero_head=True, num_block_open=network_config.num_block_open
        )
    ...
    elif network_config.name == "contrastive_net":
        backbone = get_network(network_config.backbone)
        net = ContrastiveNet(
            backbone, network_config.proj_type, network_config.hidden_layer_dim, network_config.feat_dim
        )
    elif network_config.name == "osr_branch_net":
        backbone = get_network(network_config.backbone)
        net = OSRBranchNet(backbone, network_config.hidden_layer_dim)

    elif network_config.name == "binary_net":
        backbone = get_network(network_config.backbone)
        net = BinaryNet(backbone, network_config.hidden_layer_dim)

    elif network_config.name == "bioclip":
        pass

    elif network_config.name == "dino":
        pass
