...
def mixup_criterion_group(criterion, pred, y_a, y_b, lam, group_slices):
    return lam * calc_group_softmax_loss(criterion, pred, y_a, group_slices) + (1 - lam) * calc_group_softmax_loss(
        criterion, pred, y_b, group_slices
    )
...
def calc_group_softmax_loss(criterion, logits, labels, group_slices):
    ...
    for i in range(num_groups):
        group_logit = logits[:, group_slices[i][0] : group_slices[i][1]]  # why slice on axis 1 not 0??
...
def run_eval(model, data_loader, step, group_slices, num_group):
    ...
    for train_step in tqdm(
        range(1, len(train_dataiter) + 1), desc="Test : ", position=0, leave=True, disable=not comm.is_main_process()
    ):
...
class MOSTrainer:
    ...
    def cal_group_slices(self, train_loader):
        ...
        for i in range(len(group)):
            ...
            self.classes_per_group.append(len(group[str(i)]))

    #  np.save("c-america_mos_genus_group_config.npy", np.array(self.classes_per_group))
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            y_b = y_b.cuda()
            if self.mixup > 0.0:
                c = mixup_criterion_group(self.cri, logits, y_a, y_b, self.mixup_l, self.group_slices)
            else:
