...
from .extended_trainer import ExtendedTrainer
from .contrastive_trainer import ContrastiveTrainer
from .osr_branch_trainer import OSRBranchTrainer
...
def get_trainer(net, train_loader: DataLoader, val_loader: DataLoader, config: Config):
    if type(train_loader) is DataLoader:
    ...
    elif type(train_loader) is DataFrame:
        trainers = {"contrastive": ContrastiveTrainer, "osr_branch": OSRBranchTrainer}
        return trainers[config.trainer.name](net, train_loader, config)
        ...
        return trainers[config.trainer.name](net, train_loader[0], train_loader[1], config)
