...
class OSRBranchTrainer:
    def __init__(self, net: nn.Module, train_df: DataFrame, config: Config) -> None:
        ...
        self.all_features, self.all_labels = None, None
    ...
    def train_epoch(self, epoch_idx):
        ...
        all_features_cur = torch.tensor([], dtype=torch.float32, device=device)
        ...
        for train_step in tqdm(
            ...
            logits, features1 = self.net(images1, return_feature=True)
            output = logits[labels1 < num_classes]
            target = labels1[labels1 < num_classes]
            ...
            all_features_cur = torch.cat((all_features_cur, known_embedding[mask].detach()), axis=0)
            ...
            _, features2 = self.net(images2, return_feature=True)

            score = self.net([features1, features2], return_score=True)

            # criterion = DistanceLoss()
            criterion = BinaryLoss()  # TODO: update config for criterion

            loss_cls = F.cross_entropy(output, target)
            # loss_osr = criterion(score.squeeze(1), get_taxnomic_distance(batch))
            loss_osr = criterion(score.squeeze(1), (labels1 == labels2).float())
        ...
        acc = correct / id_total

        self.train_df = df
        self.all_features = all_features_cur
...
def get_dataframe(df, y_pred, num_classes):
    ...
    image_count_map, species_index_map = species_count_map(df, num_classes)
    ref_df = get_reference_images(df, species_index_map, image_count_map, y_pred, num_classes)
...
class BinaryLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        return criterion(sigmoid, true)


def get_taxonomic_distance(batch):
