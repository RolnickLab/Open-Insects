...
class CIDERTrainer:
    def __init__(self, net: nn.Module, train_loader: DataLoader, val_loader: DataLoader, config: Config) -> None:
        ...
        #             * (1 + math.cos(math.pi * self.warm_epochs / config.optimizer.num_epochs))
        #             / 2
        #         )
        #     else:
        #         self.warmup_to = config.optimizer.lr

        self.criterion_comp = CompLoss(config.dataset.num_classes, temperature=config.trainer.trainer_args.temp).cuda()
        ...
        self.criterion_dis = DisLoss(
            ...
            self.net,
            val_loader,
            temperature=config.trainer.trainer_args.temp,
        ).cuda()
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            comp_loss = self.criterion_comp(features, self.criterion_dis.prototypes, target)
...
def adjust_learning_rate(config, optimizer, epoch):
    ...
    if config.optimizer.cosine:
        ...
        lr = eta_min + (lr - eta_min) * (1 + math.cos(math.pi * epoch / config.optimizer.num_epochs)) / 2
    else:
...
class CompLoss(nn.Module):
    ...
    def forward(self, features, prototypes, labels):
        ...
        mask = torch.eq(labels, proxy_labels.mT).float().cuda()  # bz, cls
...
class DisLoss(nn.Module):
    ...
    def __init__(self, n_cls, feat_dim, proto_m, model, loader, temperature=0.1, base_temperature=0.1):
        ...
        self.temperature = temperature
        self.base_temperature = base_temperature
        self.register_buffer("prototypes", torch.zeros(self.n_cls, self.feat_dim))
        self.model = model
        self.loader = loader
    ...
    def forward(self, features, labels):
        ...
        for j in range(len(features)):
            prototypes[labels[j].item()] = F.normalize(
                prototypes[labels[j].item()] * self.proto_m + features[j] * (1 - self.proto_m), dim=0
            )
        ...
        logits = torch.div(torch.matmul(prototypes, prototypes.T), self.temperature)

        logits_mask = torch.scatter(torch.ones_like(mask), 1, torch.arange(num_cls).view(-1, 1).cuda(), 0)
