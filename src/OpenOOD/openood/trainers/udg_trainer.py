...
class UDGTrainer(BaseTrainer):
    def __init__(
        ...
        self.k_means = KMeans(k=config.trainer.num_clusters, pca_dim=config.trainer.pca_dim)
    ...
    def _compute_loss(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            concat_cluster_id = torch.cat([batch["cluster_id"], unlabeled_batch["cluster_id"]])
            concat_cluster_reweight = torch.cat([batch["cluster_reweight"], unlabeled_batch["cluster_reweight"]])
            loss_aux = rew_ce(
                concat_logits_aux,
            ...
            with torch.no_grad():
                ...
                loss_aux_avg = loss_aux_avg * 0.8 + float(self.lambda_aux * loss_aux) * 0.2
    ...
    def _run_clustering(self, epoch_idx):
        ...
        self.train_loader.dataset.cluster_reweight = sample_weight_list[:num_train_data]
        self.train_unlabeled_loader.dataset.cluster_reweight = sample_weight_list[num_train_data:]

        print("In-Distribution Filtering (with OOD Enhancement)...", flush=True)
        old_train_pseudo_label = self.train_loader.dataset.pseudo_label
        old_unlabeled_pseudo_label = self.train_unlabeled_loader.dataset.pseudo_label
        ...
        for cluster_idx in range(self.num_clusters):
            label_in_cluster, label_counts = np.unique(old_pseudo_label[cluster_id == cluster_idx], return_counts=True)
            ...
            if np.any(purity > purity_ind_thresh):
                majority_label = label_in_cluster[purity > purity_ind_thresh][0]  # first element in the list
        ...
        self.train_unlabeled_loader.dataset.pseudo_label = new_pseudo_label[num_train_data:]
        self.train_unlabeled_loader.dataset.ood_conf = new_ood_conf[num_train_data:]
        ...
        print(
            "# Online Clustering Completed! Duration: {:.2f}s #".format(time.time() - start_time),
            flush=True,
        )
