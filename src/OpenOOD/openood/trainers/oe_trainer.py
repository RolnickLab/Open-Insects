...
class OETrainer(BaseTrainer):
    ...
    def train_epoch(self, epoch_idx):
        ...
        for train_step in tqdm(
            ...
            loss = F.cross_entropy(logits_classifier[:batch_size], batch["label"].cuda())

            loss_oe = -(
                logits_classifier[batch_size:].mean(1) - torch.logsumexp(logits_classifier[batch_size:], dim=1)
            ).mean()
