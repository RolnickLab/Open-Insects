    ...
    def __init__(self, config: Config):
        ...
        if "imagenet" in config.dataset.name:
            self.transform = tvs_trans.Compose(
                [
                    tvs_trans.RandomResizedCrop(self.image_size, interpolation=self.interpolation),
                    tvs_trans.RandomHorizontalFlip(0.5),
                    tvs_trans.ToTensor(),
                    tvs_trans.Normalize(mean=self.mean, std=self.std),
                ]
            )
        elif "ami" in config.dataset.name:
            # TODO: update data transform
            self.transform = tvs_trans.Compose(
                [
                    tvs_trans.RandomResizedCrop(self.image_size, scale=(0.3, 1)),
                    tvs_trans.RandomHorizontalFlip(),
                    tvs_trans.RandAugment(num_ops=2, magnitude=9),
                    tvs_trans.Resize((self.image_size, self.image_size)),
                    tvs_trans.ToTensor(),
                    tvs_trans.Normalize(mean=self.mean, std=self.std),
                ]
            )
