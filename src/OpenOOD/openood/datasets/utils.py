    ...
    for split in dataset_config.split_names:
        ...
        if split_config.dataset_class == "ImglistExtraDataDataset":
            ...
            batch_sampler = TwoSourceSampler(
                dataset.orig_ids, dataset.extra_ids, split_config.batch_size, split_config.orig_ratio
            )
        ...
        elif split_config.dataset_class == "AMIDataFrame":
            dataloader = pd.read_csv(split_config.imglist_pth)
...
def get_feature_opengan_dataloader(dataset_config: Config):
    ...
    for d in ["id_train", "id_val", "ood_val"]:
        # load in the cached feature
        loaded_data = load(os.path.join(feat_root, f"{d}.npz"), allow_pickle=True)
        total_feat = torch.from_numpy(loaded_data["feat_list"])
