...
class TrainOpenGanPipeline:
    ...
    def run(self):
        ...
        # init trainer
        trainer = get_trainer(net, dataloaders["id_train"], dataloaders["id_val"], self.config)
        ...
        for epoch_idx in range(1, self.config.optimizer.num_epochs + 1):
            if isinstance(dataloaders["id_train"].sampler, torch.utils.data.distributed.DistributedSampler):
                dataloaders["id_train"].sampler.set_epoch(epoch_idx - 1)

            # train the model
            net, train_metrics = trainer.train_epoch(epoch_idx)
            val_metrics = evaluator.eval_ood_val(net, id_loaders, ood_loaders, postprocessor)
