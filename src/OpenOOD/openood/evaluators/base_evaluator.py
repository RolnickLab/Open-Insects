...
class BaseEvaluator:
    ...
    def eval_acc(
        self, net: nn.Module, data_loader: DataLoader, postprocessor: BasePostprocessor = None, epoch_idx: int = -1
    ):
        net.eval()
        ...
        correct = 0
        with torch.no_grad():
            for batch in tqdm(data_loader, desc="Eval: ", position=0, leave=True, disable=not comm.is_main_process()):
                # prepare data
    ...
    def extract(self, net: nn.Module, data_loader: DataLoader, filename: str = "feature"):
        ...
        with torch.no_grad():
            for batch in tqdm(
                data_loader, desc="Feature Extracting: ", position=0, leave=True, disable=not comm.is_main_process()
            ):
                data = batch["data"].cuda()
                label = batch["label"]
